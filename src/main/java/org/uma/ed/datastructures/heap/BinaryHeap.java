...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
    ...
    BinaryHeap<T> copy = new BinaryHeap<>(that.comparator(), that.size());
  ...
  public T minimum() {
    //throw new UnsupportedOperationException("Not implemented yet");
    if (isEmpty()) {
      throw new EmptyHeapException("not minimun in empty tree");
    }
  ...
  private void heapifyDown(int index) {
    //throw new UnsupportedOperationException("Not implemented yet");
    while (!isLeaf(index)) { //not leaf at least 1 child (left)
      int indexChild = leftChild(index);
      int indexRightChild = rightChild(index); //try to get right child
      //if there's a right child and is less than left, update indexChild
      if (isNode(indexRightChild) && lessThan(indexRightChild, indexChild)) {
        indexChild = indexRightChild;
      }
      //indexChild is now minimum child
      if (lessThan(indexChild, index)) { //if child less than parent swap
  ...
  public void deleteMinimum() {
    //throw new UnsupportedOperationException("Not implemented yet");
    if (isEmpty()) {
      throw new EmptyHeapException("deleteMinimun on empty tree");
    }
