...
public class LinkedList<T> extends AbstractList<T> implements List<T> {
  ...
  public void insert(int index, T element) {
    if (index == size) {
      ...
      validateIndex(index);
      Node<T> previous = atIndex(index - 1); //locate previous one
      Node<T> node = new Node<>(element, previous.next); // next to previous will be after
      previous.next = node; // previous should be followed by new node
      size++;
    }
  }
