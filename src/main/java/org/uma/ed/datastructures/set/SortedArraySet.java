...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  /* Me: <T> define un parámetro de tipo genérico llamado T.
    extends Comparable<? super T> significa que T debe implementar la interfaz Comparable,
    lo que implica que cualquier objeto de tipo T puede ser comparado con otros objetos de tipo T o con sus supertipos.
    El uso de ? super T permite que T sea comparado con cualquier supertipo de T.
  */
  ...
  public void insert(T element) {
    //throw new UnsupportedOperationException("Not implemented yet");
    Finder finder = new Finder(element); //O(log n)
    if(!finder.found) {
      ensureCapacity(); //O(n)
      //finder.index is where we have to insert the element
