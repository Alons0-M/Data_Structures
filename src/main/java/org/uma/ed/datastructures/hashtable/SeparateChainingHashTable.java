...
public class SeparateChainingHashTable<K> implements HashTable<K> {
  ...
  public static <K> SeparateChainingHashTable<K> copyOf(SeparateChainingHashTable<K> that) {
    ...
    SeparateChainingHashTable<K> newHashTable = new SeparateChainingHashTable<>(that.table.length, that.maxLoadFactor);
    for (K key : that) {
  ...
  public void insert(K key) {
    ...
    if (finder.current != null) {
      ...
      //element not in table, insert
      Node<K> node = new Node<>(key, table[finder.index]);
      table[finder.index] = node; // insert at the beginning of the list
    }
  }
  ...
  public void delete(K key) {
    ...
    if (finder.current != null) {
      ...
      if (finder.previous == null) {
        //node to delete is first one in the list
  ...
  public void clear() {
    //throw new UnsupportedOperationException("Not implemented yet");
    for (int i = 0; i < table.length; i++) { //isn't O(n), is a constant O(1)
      table[i] = null;
    }
    size = 0;
  }
