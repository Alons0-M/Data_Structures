...
public class LinearProbingHashTable<K> implements HashTable<K> {
  ...
  private int searchIndex(K key) {
    ...
    return index; //if keys[index] == null, not in table but should be inserted, !=
  }
  ...
  public void insert(K key) {
    ...
    if (keys[index] == null) {
      //key already not in table, not overwrite
      size++;
    }
    keys[index] = key;
  }
  ...
  public K search(K key) {
    //throw new UnsupportedOperationException("Not implemented yet");
    int index = searchIndex(key);
    return keys[index]; //if null return null
  }
  ...
  public void delete(K key) {
    ...
    if (keys[index] != null) {
      ...
      while (keys[index] != null) {
        ...
        index = advance(index); //go to next key in cluster
      }
    }
  }
